...
MPU6050::MPU6050(TwoWire &w) { // @suppress("Class members should be properly initialized")
	wire = &w;
	accCoef = 0.02f;
	gyroCoef = 0.98f;
}

MPU6050::MPU6050(TwoWire &w, float aC, float gC) { // @suppress("Class members should be properly initialized")
	wire = &w;
	accCoef = aC;
	gyroCoef = gC;
}
 ...
 */
void MPU6050::writeBits(uint8_t regAddress, uint8_t startBit, uint8_t length,
		uint8_t data) {
...
void MPU6050::setGyroSensitivity(uint8_t range) {
	...
	writeBits(MPU6050_GYRO_CONFIG, MPU6050_GYRO_CONFIG_FS_SEL_BIT,
			MPU6050_GYRO_CONFIG_FS_SEL_LENGTH, range);
}
...
void MPU6050::setAccelSensitivity(uint8_t range) {
	...
	writeBits(MPU6050_ACCEL_CONFIG, MPU6050_ACCEL_CONFIG_FS_SEL_BIT,
			MPU6050_ACCEL_CONFIG_FS_SEL_LENGTH, range);
}
